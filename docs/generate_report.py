...
def build():
    ...
    story += [
        ...
        Paragraph("Python 3.12, Docker, Minikube, kubectl, Git, GitHub CLI (gh).", BODY),
        Spacer(1, 0.2*cm),
        Paragraph("Quick Start — Local", H3),
        Paragraph("git clone https://github.com/sampathshetty85/heart-disease-mlops.git", CODE),
        Paragraph("cd heart-disease-mlops", CODE),
        Paragraph("python -m venv .venv &amp;&amp; source .venv/bin/activate", CODE),
        Paragraph("pip install -r requirements.txt", CODE),
        Paragraph("python src/data/download.py      # fetches UCI dataset", CODE),
        Paragraph("python src/data/preprocess.py    # cleans and splits data", CODE),
        Paragraph("python src/models/train.py       # trains 3 models, logs to MLflow", CODE),
        Paragraph("uvicorn src.api.main:app --port 8000  # start API", CODE),
        Spacer(1, 0.3*cm),
        Paragraph("Docker — Build &amp; Run", H3),
        Paragraph("docker build -t heart-disease-mlops:latest .", CODE),
        Paragraph("docker run -p 8000:8000 heart-disease-mlops:latest", CODE),
        ...
        Paragraph('     -d \'{"age":45,"sex":1,"cp":0,"trestbps":120,"chol":200,"fbs":0,', CODE),
        Paragraph('           "restecg":0,"thalach":160,"exang":0,"oldpeak":0.0,"slope":2,"ca":0,"thal":2}\'', CODE),
        Spacer(1, 0.3*cm),
        Paragraph("Kubernetes — Minikube", H3),
        ...
        Spacer(1, 0.3*cm),
        Paragraph("Monitoring Stack (Docker Compose)", H3),
        Paragraph("docker-compose up -d   # starts api + prometheus + grafana", CODE),
        Paragraph("# API: localhost:8001  Prometheus: localhost:9090  Grafana: localhost:3000", CODE),
        Spacer(1, 0.3*cm),
        Paragraph("CI/CD", H3),
        Paragraph(
